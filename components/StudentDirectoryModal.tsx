...
export const StudentDirectoryModal: React.FC<Props> = ({ onClose }) => {
  ...
  const handleDelete = () => {
    if (selectedStudentId) {
        ...
        handleBackToList();
        setFormData({ name: '', username: '', password: '', phoneNumber: '', hasPaid: false, avatarUrl: '' });
    }
  };
        ...
        <div className={`w-full md:w-1/3 border-r border-gray-200 bg-gray-50 flex flex-col ${isEditing ? 'hidden md:flex' : 'flex h-full'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2 pb-20">
                ...
                >
                    ...
                    <span className="font-bold text-base">新增學生</span>
                </button>
                    ...
                    <div className="space-y-8 max-w-lg mx-auto sm:mx-0">
                        ...
                        <div className="p-4 sm:p-6 bg-gray-50 rounded-2xl border border-gray-200 shadow-sm">
                            ...
                            <div className="flex bg-white rounded-xl p-1.5 border border-gray-200 mb-6">
                                <button 
                                    ...
                                    className={`flex-1 py-3 text-sm font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${formData.membershipType === 'CREDIT' ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
                                >
                                    <Coins size={18} />
                                    點數扣點制
                                </button>
                                <button 
                                    ...
                                    className={`flex-1 py-3 text-sm font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${formData.membershipType === 'UNLIMITED' ? 'bg-green-100 text-green-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
                                >
                                    <Infinity size={18} />
                                    課程自由
                                </button>
                            ...
                            {formData.membershipType === 'UNLIMITED' ? (
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full p-4 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none text-gray-900 text-lg"
                                    />
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 sm:gap-4">
                                        <button 
                                            onClick={() => setFormData(p => ({...p, credits: Math.max(0, (p.credits || 0) - 1)}))} 
                                            className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl border bg-white hover:bg-gray-50 text-gray-600 text-lg sm:text-xl font-bold flex items-center justify-center"
                                        >
                                            -
                                        </button>
                                        <input 
                                            ...
                                            className="flex-1 min-w-0 p-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 text-center font-bold text-xl sm:text-2xl"
                                        />
                                        <button 
                                            onClick={() => setFormData(p => ({...p, credits: (p.credits || 0) + 1}))} 
                                            className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl border bg-white hover:bg-gray-50 text-gray-600 text-lg sm:text-xl font-bold flex items-center justify-center"
                                        >
                                            +
                                        </button>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">帳號 (ID)</label>
                                    <div className="relative">
                                ...
                                <div className="bg-yellow-50 p-4 rounded-xl border border-yellow-100">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="text-sm text-yellow-800">
                                            <p>學生忘記密碼？</p>
                                            <p className="text-xs mt-1 opacity-80">可重置為預設密碼 "123456"</p>
                                        </div>
                                        {showResetConfirm ? (
                                            <button 
                                                type="button"
                                                onClick={handleResetPassword}
                                                className="flex items-center gap-2 bg-red-600 text-white border border-red-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-700 shadow-sm animate-in fade-in zoom-in duration-200"
                                            >
                                            ...
                                            <button 
                                                ...
                                                className="flex items-center gap-2 bg-white border border-yellow-300 text-yellow-700 px-4 py-2 rounded-lg text-sm font-bold hover:bg-yellow-100"
                                            >
                                                <KeyRound size={16}/>
                                                重置密碼
                                            </button>
