  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
        ...
        img.onload = () => {
            ...
            if (ctx) { ctx.clearRect(0, 0, SIZE, SIZE); ctx.drawImage(img, sx, sy, minSide, minSide, 0, 0, SIZE, SIZE); }
        ...
        <div className="bg-zen-600 p-6 text-white relative shrink-0">
            ...
            <div className="flex items-center gap-4">
                <div className="relative group shrink-0">
                    <div className="w-20 h-20 rounded-full shadow-xl overflow-hidden bg-white/20 relative flex items-center justify-center ring-1 ring-white/30">
                        {isUploading ? (
                            <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-20"><Loader2 className="animate-spin text-white" /></div>
                        ) : (
                            currentUser.avatarUrl ? (
                                <img src={currentUser.avatarUrl} alt={currentUser.name} className="w-full h-full object-cover"/>
                            ) : (
                                isAdmin ? <UserCog size={32} className="text-white/50" /> : <UserIcon size={32} className="text-white/50" />
                            )
                        )}
                    </div>
                ...
                <div className="flex-1 min-w-0 pr-10">
                    {isEditing ? (
                        <div className="space-y-2">
                            <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} className="w-full px-2 py-1 text-gray-900 rounded text-lg font-bold outline-none border border-white/50 focus:ring-2 focus:ring-white/50 bg-white/90" placeholder="姓名" autoFocus />
                            <div className="flex items-center bg-white/20 rounded px-2 py-1"><Phone size={14} className="text-white/70 mr-2" /><input type="tel" value={newPhone} onChange={(e) => setNewPhone(e.target.value)} className="w-full bg-transparent text-sm text-white placeholder-white/50 outline-none" placeholder="電話號碼" /></div>
                            <button onClick={handleSaveProfile} className="bg-white/20 hover:bg-white/30 text-white text-xs px-3 py-1.5 rounded-full flex items-center gap-1 mt-1 font-bold"><Save size={12} /> 儲存</button>
                        </div>
                    ) : (
                        <div className="group">
                            <div className="flex items-center gap-2 mb-1"><h2 className="text-2xl font-bold truncate">{currentUser.name}</h2><button onClick={() => setIsEditing(true)} className="opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-white/20 rounded shrink-0"><Edit2 size={14} /></button></div>
                            <p className="text-zen-100 text-sm font-medium">@{currentUser.username}</p>
                            {liveStudentData.phoneNumber && <p className="text-zen-100 text-xs mt-1 flex items-center gap-1"><Phone size={12}/> {liveStudentData.phoneNumber}</p>}
                        </div>
                    )}
                    {!isEditing && isStudent && (
                        <div className={`inline-flex items-center gap-1.5 mt-2 px-3 py-1.5 rounded-xl text-xs font-bold ${membershipType === 'UNLIMITED' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>
                            {membershipType === 'UNLIMITED' ? <Infinity size={14} /> : <Coins size={14} />}
                            {membershipType === 'UNLIMITED' ? (
                                <div className="flex flex-col leading-tight"><span className="whitespace-nowrap">課程自由</span><span className="text-[10px] opacity-80 font-medium">至 {expiry || '未設定'}</span></div>
                            ) : (
