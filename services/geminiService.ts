...
export const generateSubstitutionNotification = async (
  ...
  try {
    // FIX: Always use a named parameter object for GoogleGenAI initialization.
    // Updated model to 'gemini-3-flash-preview' for basic text tasks.
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    
    const prompt = `
      你是一個 "ZenFlow 瑜伽" 教室的熱心且有禮貌的櫃檯經理。
      請用「繁體中文 (台灣)」寫一則簡短、溫慢且正面的通知訊息（最多 50 字）給學生。
    ...
    // FIX: Using response.text directly as a property (not a method).
